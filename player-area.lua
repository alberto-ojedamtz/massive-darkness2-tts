...
function setUpPlayerAreas()
    for _, playerMenu in pairs(PLAYERS_PROPERTIES) do
...
function placeObjectRelativeToPlayerBoard(playerProperties, object, relativeValues, lock)
    local position = getObjectFromGUID(PLAYER_BOARD_GUIDS[playerProperties.number]).getPosition()

    local xPos = position.x + relativeValues.x
    local yPos = position.y + relativeValues.y
    local zPos = position.z + relativeValues.z

    if (playerProperties.number > 2) then
